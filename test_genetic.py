...
class TestGA (unittest.TestCase):
    def test_arith_crossover (self):
        ...
        self.assertIsNone (genetic.arith_crossover(population, 0, 0)[0])

        prob_cross = 1.0
        c = 0.2
        cross_pop, parents = genetic.arith_crossover(
    ...
    def test_mutation (self):
        ...
        np.testing.assert_equal (
            genetic.arith_mutation(chromosomes, prob_mut), chromosomes)

        array_to_compare = np.zeros((3, 3))
        prob_mut = 0.5

        np.testing.assert_allclose (
            genetic.arith_mutation(chromosomes, prob_mut), 
            array_to_compare, atol=1.0
        )
        np.testing.assert_allclose (
            genetic.arith_mutation (
    ...
    def test_roulette_selection (self):
        ...
        pop_selected = genetic.roulette_selection(population, n_to_select,
